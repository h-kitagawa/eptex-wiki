...
> * [https://twitter.com/zr_tex8r/status/1119952801952485377 ZR さんの Twitter]，[ptex-manual/4](https://github.com/texjporg/ptex-manual/issues/4)
...
> * [https://twitter.com/zr_tex8r/status/1119966520321265664 ZR さんの Twitter]，[tex-jp-build/68](https://github.com/texjporg/tex-jp-build/issues/68)
...
> * [https://oku.edu.mie-u.ac.jp/tex/mod/forum/discuss.php?d=852 TeX Forum 中の記事]（前段階），[tex-jp-build/82](https://github.com/texjporg/tex-jp-build/issues/82)
...
> * TeX Live 2019 以前の ptexenc では，（必要なバイト数だけ続いていない）不正な UTF-8 シーケンスを U+0000 と解釈し，さらにこれが「JIS コードで 222F の文字」に変換[[Footnote(ptexenc にある JIS X 0208 から Unicode の変換テーブルでは，JIS X 0208 未定義文字には 0 と記録されています．ptexenc はこのテーブルを逆引きして Unicode から JIS X 0208 への変換を行う（ただし configure 時に iconv を使用可能にできる）ので，U+0000 から JIS X 0208 の最初の未定義文字 JIS 222F に変換されるというわけです．)]]されて pTeX の内部処理にかかる
...
元々 !LaTeX のフォーマットファイルは 4 MB 強程度の容量があったのですが，[https://www.latex-project.org/news/2020/02/02/issue31-of-latex2e-released/ LaTeX 2020-02-02] で expl3 がプリロードされることになったことに伴い，`latex` フォーマット (pdfTeX) は 8.3 MB，`platex` フォーマット (e-pTeX) は 10.6 MB と大幅に増大してしまいました[[Footnote(ちなみに，`latex` と `platex` で 2 MB 以上もの差ができているのは，①TFM 情報を格納する配列 *font_info* の 1 要素が pdfTeX では 4 バイトなのに対し，pTeX 系列では 8 バイトである，②expl3 は読み込み時に 65536 個の `\fontdimen` を持つフォントを 8 個定義する，③「65536 個の `\fontdimen` を持つフォント」はそれぞれ *font_info* を約 65000 要素使う，というのが主な理由です．)]]．
...
> * [https://www.tug.org/svn/texlive?view=revision&revision=53729 TeX Live svn r53729]
...
\input "h o g e".tex" "a
```
は両者とも 「`h o g e.tex a`」というファイルを読み込むようになっています[[Footnote(エラーメッセージや最後の `Transcript written on ...` で表示されるファイル名では全体が「`"`」で囲まれますが，これはファイル名出力の際に TeX が付加するものです．)]]．
...
> * [https://twitter.com/zr_tex8r/status/1189063963381600257 ZR さんの Twitter]，[tex-jp-build/95](https://github.com/texjporg/tex-jp-build/issues/95)
...
なお，欧文文字，つまり文字コードが 0--255 に限られる場合は，同様のことはマクロで行なえます（上に挙げた ZR さんの Twitter，[https://tex.stackexchange.com/questions/296211/expansion-of-char-in-def StackExchange の記事]中のコメントを参照）．
...
#### コメント ====
[[LineComment]]
>  * おっと，早とちりしていました，修正しました．[[BR]]-- [h7k](id:h7k) (2020-12-19 19:11:41 JST)
>  * \currentcjktoken は正しくは \enablecjktoken ならば 0，\disablecjktoken ならば 1 です。[[BR]]-- [aminophen](id:aminophen) (2020-12-19 13:36:05 JST)
