...
それぞれの後半部の「」の量は，フォントメトリックによって決められています．和文フォントメトリック（[pTeX 公式ページ内のファイルフォーマットの解説](https://asciidwango.github.io/ptex/tfm/jfm.html)）では，和文文字をいくつかの文字クラスにグループ化し，文字クラスごとに寸法・空白の指定をすることになっています．
 ...
 * (b) 文字クラス 0 の文字と開き括弧「『」の間には半角空きが入る
 * (c) 全角カンマ「，」と文字クラス 0 の文字の間にも半角空きが入る
 * (d) 全角カンマと開き括弧「『」の間には半角空きが入る
...
[^1]: 以降のベースラインを `x.y` ptだけ下にずらす役割を持ちます．
```tex
 ...
 * 間にサイズ変更命令を挟んだ `，\Large \fixjfmspacing 「` といった入力は予期しない
  結果[^2]になります．
[^2]: そもそもこの場合の「正しい結果」って何なんでしょう．
   * これは `\fixjfmspacing` 実行時のカレントフォントを用いて補正量を計算する[^3]ためです．
[^3]: 文字コードと同様に，「最後の和文文字のフォント」を知る方法は現時点ではなく，新たに `\lastnodefont` とでもいう命令を作る以外にないでしょう（名前が良くない……）．
...
## 番外：LuaTeX-ja の場合
LuaTeX や，その上で動く [LuaTeX-ja](https://ctan.org/pkg/luatexja) の場合には，状況が異なります．
...
ノード的に言えば，`\MC` の「，」の直後に `\GT` の「『」が続いている[^4]わけですが，両者は同じメトリックなので，
両者の間には（「，」と「『」の間に入るべき）半角空きが入るようになっています．
[^4]: `\showlists` で見ると，`\tenrm ，` と `\tenrm 『` のように，欧文フォントを用いているように見えます．しかし，実際には `\showlists` では見えないところに使っている和文フォントの情報を格納しており，問題ありません．
```tex
% plain LuaTeX
